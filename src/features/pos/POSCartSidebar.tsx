...
export const POSCartSidebar: React.FC<POSCartSidebarProps> = ({
  ...
  const handleCommitNote = (targetId: string) => {
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-3 sm:p-4 space-y-2.5 custom-scrollbar">
        {items.length === 0 ? (
          ...
          items.map((item, index) => {
            const targetId = item.id || item.productId || `item-${index}`;
            const isEditing = editingNoteId === targetId;
              ...
              >
                ...
                <div className="flex items-center justify-between pt-2 mt-2 border-t border-stone-100">
                  ...
                  <div className="flex items-center gap-1.5 bg-white px-2 py-0.5 rounded-xl border border-stone-200">
                    <button
                      onClick={() => onUpdateQuantity(targetId, -1)}
                ...
                {isEditing && (
                  <div className="mt-2 pt-2 border-t border-amber-200 flex gap-1.5">
                    ...
                    />
                    <button
                      onClick={() => handleCommitNote(targetId)}
